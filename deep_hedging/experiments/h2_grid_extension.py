...
Extends Part A of Phase 9 (Pareto front) to finer resolution on
BOTH axes, using BS delta only (no training). Reuses existing
Phase 9 Part A cells where the grids overlap.

Input:
    figures/pareto_part_A_results.json   (Phase 9 Part A scalar results)
...
# ─── Fixed rBergomi parameters (match Phase 9 Part A) ─────────
...
def load_prompt_9_cells(path: Path) -> dict[int, dict[float, dict]]:
    """Load BS delta cells from Phase 9 Part A that overlap with our grid.

    Returns nested dict ``{n_steps: {cost_lambda: {...}}}``.
    """
    if not path.exists():
        print(f"  Phase 9 JSON not found at {path} — will compute all cells fresh",
              flush=True)
        return {}
...
def generate_paths_for_freq(
    ...
    # cell in Phase 9 where no training data is involved).
...
def run_extended_grid(
    ...
    # Load cached Phase 9 BS cells for reuse
    ...
    if reuse_prompt_9:
        ...
        if prompt_9_cells:
            ...
            print(f"  Loaded {total_reused} BS cells from Phase 9 Part A", flush=True)
    ...
    for n_steps in freq_values:
        ...
        # Determine which BS costs are already cached from Phase 9
